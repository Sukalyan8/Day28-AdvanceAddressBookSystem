    ...
    class Program
    {
        ...
        public static void SortContactPerson()
        {
            ...
            foreach (KeyValuePair<string, List<AddressBook>> kvp in NumberNames)
            {
                ...
                CompareContactPerson contactPersonComparer = new CompareContactPerson();
                switch (option)
                {
                    case 1:
                        //Set field based on the switch case Option
                        contactPersonComparer.compareThroughFields = CompareContactPerson.sortBy.firstName;
                        //Call Sort Method
                        listAddressBook.Sort(contactPersonComparer);
                        break;
                    case 2:
                        contactPersonComparer.compareThroughFields = CompareContactPerson.sortBy.state;
                        listAddressBook.Sort(contactPersonComparer);
                        break;
                    case 3:
                        contactPersonComparer.compareThroughFields = CompareContactPerson.sortBy.city;
                        listAddressBook.Sort(contactPersonComparer);
                        break;
                    case 4:
                        contactPersonComparer.compareThroughFields = CompareContactPerson.sortBy.zip;
                        listAddressBook.Sort(contactPersonComparer);
                        break;
                }
